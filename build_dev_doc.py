...
def build_document() -> None:
    ...
    add_table(
        ...
        ["指标", "采集口径（Linux）", "展示与告警"],
        [
            ["CPU 使用率", "/proc/stat 两次采样差分，区分 user/system/iowait；默认 1 秒间隔。", "总使用率 KPI + 60 分钟折线；>85% 持续 5 分钟警告。"],
            ["内存使用率", "/proc/meminfo；used = total - available，兼容无 MemAvailable 的旧内核。", "已用/总量、百分比、Swap；>90% 警告。"],
            ["系统负载", "/proc/loadavg 的 1/5/15 分钟值，并按逻辑 CPU 核数归一化。", "三条曲线；1 分钟归一负载 >1.0 警告。"],
            ["磁盘使用率", "优先解析 df -P -B1；排除 tmpfs、devtmpfs，可配置挂载点。", "容量、已用、剩余、挂载点；>80% 警告、>90% 严重。"],
