...
- 推送与项目版本一致的标签（例如 `v0.2.44`）：构建全部平台，并自动创建或更新 GitHub Release。

自动发布包含 macOS arm64/x86_64 DMG、Windows x64 NSIS EXE/便携 ZIP、Linux x86_64 tar.gz 和统一的 SHA-256 校验文件。Windows 使用 `vcpkg` 的 `x64-windows-static-md` triplet 静态链接 libssh2/zlib，避免安装后缺少非 Qt DLL；Qt 运行库和插件由 CMake 的 Qt 部署脚本写入安装包。
...
- Linux 包需在目标发行版验证 Qt 运行库、`libsecret-tools` 与系统密钥环。
